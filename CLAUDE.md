...
- **Status colour code:** `--s-done` (aqua), `--s-progress` (blue), `--s-blocked` (orange) and `--s-backlog` (violet) colour the column tops, summary dots and charts. The set was checked for colour-blind separation, and colour is always paired with a text label. Priority keeps its own colours (card left border and pill). Don't reuse one set for the other.
- **Portfolio overview** (`renderCharts()`, called from `renderBoard()`) charts the same filtered list as the board. It shows a completion ring (Done ÷ total) with a status legend, plus bars stacked by status grouped by project, category, assignee or priority (`CHART_GROUPS`). Bars stack in `STACK_ORDER` (Done first) and have a hover tooltip. A Table view is the accessible and exact-numbers alternative. The chart grouping, table view and hidden/shown setting live in `state.ui`.
- **"Today's focus" pop-up** (`#focus-dialog`, `initFocusPrompt()`) opens `FOCUS_PROMPT_DELAY_MS` after load and again from the header's "★ My focus" button. `rankOpenTasks()` orders open tasks by overdue first, then priority, then soonest due, and the top `FOCUS_TOP_N` are shown. There's no sign-in, so the visitor picks their name from the assignees ("I am…", `state.ui.visitor`, page load only) to see their own overdue tasks. `renderFocus()` runs inside `renderBoard()`, so it stays live while open. `showTaskOnBoard()` clears filters if needed, then scrolls to and highlights the card.
- **Support pop-up:** `initSupportPrompt()` opens `#help-dialog` once per page load, after `SUPPORT_PROMPT_DELAY_MS` (10 s) of time with the tab visible. It waits while any other dialog is open. The hotline number is `SUPPORT_HOTLINE` in the config block. This is page code; Claude Code hooks in `.claude/hooks` can't affect site visitors.
